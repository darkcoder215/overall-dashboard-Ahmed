...
const Index = () => {
  ...
  return (
    <div className="dark flex h-screen flex-col bg-background text-foreground" dir="rtl">
      ...
      <header className="animate-fade-in flex items-center justify-between bg-black px-3 py-2 sm:px-6 sm:py-3">
        {/* Right side: Logo + brand name + user info */}
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <div className="flex h-8 w-8 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 p-1 sm:p-1.5 transition-transform duration-200 hover:scale-105">
            <img src={`${import.meta.env.BASE_URL}thamanyah.png`} alt="ثمانية" className="h-full w-full object-contain" />
          </div>
          <div className="min-w-0">
            <h1 className="font-display text-xs sm:text-sm font-bold leading-tight text-white truncate">
              مساعد ثمانية الذكي
            </h1>
            <div className="flex items-center gap-1 sm:gap-2">
              <span className="font-ui text-[10px] sm:text-xs text-white/60 truncate">{user.display_name}</span>
              {/* Highlight badge per brand "highlight style" */}
              <span className="rounded-full bg-brand-green/20 px-1.5 sm:px-2 py-0.5 font-ui text-[9px] sm:text-[10px] font-medium text-brand-green shrink-0">
